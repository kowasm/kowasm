...
fun Appendable.tagClass(tag: TagInfo, excludeAttributes: Set<String>) {
    ...
    val parentElementIfaces = tag.tagGroupNames.map {
        it.humanize().replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() }
    }
...
internal fun Appendable.tagAttributeVar(
    ...
    if (attribute.type == AttributeType.ENUM) {
        options.add(ReferenceConst(attribute.enumTypeName.replaceFirstChar { it.lowercase() } + "Values"))
...
private fun tagCandidates(tag: TagInfo) =
    (listOf(tag.memberName) + tagReplacements.map { tag.memberName.replace(it.first.toRegex(), it.second) }).flatMap {
        listOf(
...
fun Appendable.consumerBuilderJS(tag: TagInfo, blockOrContent: Boolean) {
    ...
    function(
        tag.memberName,
        tagBuilderFunctionArguments(tag, blockOrContent),
        resultType,
        receiver = "TagConsumer<HTMLElement>"
    )
...
fun Appendable.htmlTagEnumBuilderMethod(
    ...
    enumAttribute.enumValues.forEach { enumValue ->
        ...
        if (tagBuilderCouldBeInline(tag, blockOrContent)) append("inline ")
        function(
            enumValue.fieldName + tag.memberName.replaceFirstChar { if (it.isLowerCase()) it.titlecase() else it.toString() },
            arguments,
            "Unit",
            receiver = receiver
        )
